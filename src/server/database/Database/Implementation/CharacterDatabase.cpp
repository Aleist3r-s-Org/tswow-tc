...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_DEL_ACCOUNT_TRANSMOG, "DELETE FROM account_transmog WHERE accountid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_ACCOUNT_TRANSMOG, "SELECT `type`, entry FROM account_transmog WHERE accountid = ? ORDER BY entry", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ACCOUNT_TRANSMOG, "INSERT INTO account_transmog (`accountid`, `type`, `entry`) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_TRANSMOG_SETS, "DELETE FROM `transmogrification_sets` WHERE owner = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_TRANSMOG_SETS, "SELECT `presetid`, `setname`, `setdata` FROM `transmogrification_sets` WHERE owner = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_TRANSMOG_SETS, "REPLACE INTO `transmogrification_sets` (`owner`, `presetid`, `setname`, `setdata`) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    /** @custom-end */
